...
#include "defines.h"
#include "statics.h"
#ifdef __APPLE__	// macOS does not provide <malloc.h>. malloc() is defined in <stdlib.h>.
#include <stdlib.h>
#else
#include <malloc.h>
#endif
...
public:
	...
	void Render(float *inl, float* inr, int samples, float dry, float wet)
	{
		...
		for(i = 0; i < samples; i++)
		{
			...
			out_L = this->bufl[d1] + f0 * (this->bufl[d0] - this->bufl[d1]);
			...
			out_R = this->bufr[d1] + f0 * (this->bufr[d0] - this->bufr[d1]);

			tmp = inl[i] + this->Feedback * out_L;
			UNDENORM(tmp);
			this->bufl[this->Position] = tanhf(tmp);

			tmp = inr[i] + this->Feedback * out_R;
			UNDENORM(tmp);
			this->bufr[this->Position] = tanhf(tmp);

			inl[i] = out_L * wet + inl[i] * dry;
			inr[i] = out_R * wet + inr[i] * dry;
