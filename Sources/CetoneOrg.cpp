...
: DISTRHO::Plugin(pNumParameters, 0, 0)
{
	...
	// Explicitly set sample rate and block size here, otherwise CO will crash!
	this->setSampleRate(getSampleRate());
	this->setBlockSize(getBufferSize());
	...
	this->curProgram = 0;
	//this->LoadPresets("autoload.fxb");	// NOTICE: Default bank is missing!
	this->ReadProgram();
...
VstInt32 CO::processEvents(const DISTRHO::MidiEvent* events, uint32_t eventCount)
{
#if 0
	...
	for (VstInt32 i = 0; i < eventCount; i++) {
		...
		this->mStack->Push(midiData[0], midiData[1] & 0x7f, midiData[2] & 0x7f, _event.frame);
	}
	return 1;
#endif
}
...
void CO::myProcess(float **inputs, float **outputs, VstInt32 sampleFrames, bool replace)
{
	...
	if(replace)
	{
		for(i = 0; i < sf; i++)
		{
			out_L[i] = this->_Synth->fLeft[i];
			out_R[i] = this->_Synth->fRight[i];
		}
	}
	else
	{
...
float CO::getParameter(VstInt32 index) const
{
	switch (index)
	{
	default:		return 0.0f;
...
#else
			memcpy(&(this->_prg[this->curProgram]), data, sizeof(PROGRAM));
#endif
...
#else
			memcpy(&(this->_prg[0]), data, sizeof(PROGRAM) * 128);
#endif
			this->SetVersion();
			this->ReadProgram();
			return (VstInt32)sizeof(PROGRAM) * 128;
...
#else
	if(nr > -1)
		snprintf(name, kVstMaxProgNameLen, "CetoneOrg #%d", nr);
	else
		snprintf(name, kVstMaxProgNameLen, "CetoneOrg");
#endif
