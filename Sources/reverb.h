...
public:
	CombFilter()
	: _Pos(0)	// AnClark MOD: Must explicitly set _Pos to 0, when building with GCC!
	{
		this->Reset();
	}
...
public:
	...
	void Render(float *left, float* right, int samples)
	{
		...
		for(i = 0; i < samples; i++)
		{
			...
			out_L = this->_combs[0]->Process(input)
			...
			out_R = this->_combs[1]->Process(input)
			...
			out_L = this->_alls[0]->Process(out_L);
			out_L = this->_alls[2]->Process(out_L);
			out_L = this->_alls[4]->Process(out_L);
			out_L = this->_alls[6]->Process(out_L);
			out_R = this->_alls[1]->Process(out_R);
			out_R = this->_alls[3]->Process(out_R);
			out_R = this->_alls[5]->Process(out_R);
			out_R = this->_alls[7]->Process(out_R);

			left[i] = out_L * this->_Wet1 + out_R * this->_Wet2 + left[i] * this->_Dry;
			right[i] = out_R * this->_Wet1 + out_L * this->_Wet2 + right[i] * this->_Dry;
		}
	}
private:
	void Update()
